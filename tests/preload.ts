...
const SCRATCH_CACHE_HOME = mkdtempSync(join(tmpdir(), "pdf-bucket-test-cache-"));
process.env.XDG_CACHE_HOME = SCRATCH_CACHE_HOME;

// A hook registered in the preload runs once, after every test file of the process.
afterAll(() => {
  rmSync(SCRATCH_DATA_HOME, { recursive: true, force: true });
  rmSync(SCRATCH_CACHE_HOME, { recursive: true, force: true });
});
