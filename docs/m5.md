...
| Action | Result |
| --- | --- |
| start the app, three times | `/status` answers after 118, 125 and 138 ms; the first `/api/library` after 458–512 ms (the store reads the four stored PDFs); `pdf-bucket-desktop` itself holds the port |
...
The last row was recorded by removing the checkout's allow entry (the `allowPath` that `direnv status` names). `direnv deny` does not block the file in direnv 2.37.1: the allow entry stays, and `direnv export json` and `direnv exec` load the `.envrc` and exit 0.
