...
## Launcher entry and tray

`just provision` also installs `desktop/pdf-bucket-desktop.desktop`, with `Exec` set to the installed binary, as `$XDG_DATA_HOME/applications/pdf-bucket-desktop.desktop`. It installs the app icon as `pdf-bucket-desktop` in the `hicolor` theme at 32×32 and 128×128. `desktop-file-validate` checks the entry during provisioning. The entry and icon are named after the window's Wayland app_id, the binary name.

The desktop binary (`desktop/src-tauri/src/lib.rs`):
...
- **Close.** Closing the window hides it (`CloseRequested` → `prevent_close` + `hide`). The process keeps its event stream and its tray icon.
- **Quit.** The tray item exits with status 0. `Restart=on-failure` does not restart the unit after that.
- **Second launch.** `tauri-plugin-single-instance` holds `com.dzackgarza.pdfbucket.SingleInstance` on the session bus. A second launch exits and the running process shows its window.
...
Recorded on 2026-09-24 with the unit's window. Chromium had focus on the same workspace, and events were read from the Hyprland event socket. The tray menu was driven through its `com.canonical.dbusmenu` `Event` method, which is the call a click makes.
...
| close the window (`hl.dsp.window.close`) | no mapped window; process and tray item alive; unit `active` |
...
| tray Quit | no process; unit `inactive`, `Result=success`, exit status 0; tray item gone |
| `gtk-launch pdf-bucket-desktop` with nothing running | one process, window mapped on the bucket origin |
