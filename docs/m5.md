...
### The window at login needs `graphical-session.target`

systemd starts `graphical-session.target` only when the session asks for it (systemd.special(7)); a compositor launched through `uwsm`, or a session unit that `BindsTo=graphical-session.target`, does.
This Hyprland session starts its programs from `~/.config/hypr/confs/startup.lua` and never reaches the target (`systemctl --user status graphical-session.target` is `inactive (dead)`), so at the next login the server comes up and the window does not.
The window unit is not moved to `default.target`: the user manager reaches it before the compositor exists, so the window would start without a display.
