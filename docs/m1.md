...
$ just test-capture
 24 pass
 0 fail
Ran 24 tests across 1 file. [50.43s]
```
...
| --- | --- | --- | --- |
| arXiv `/pdf/2401.00001` (no `.pdf`, `application/pdf`) | intercepted | `2401.00001.pdf` stored | `2401.00001.pdf` stored |
...
| `<iframe>` 1000×700 | intercepted | `chapter.pdf` stored | `chapter.pdf` stored |
...
For each intercepted case, the suite reads the stored file's embedded provenance through the store (`pdfbucket describe`). It checks `pdf_url`, `source_url` (the linking page), `title_hint` (the link text), and the SHA-256 of the served bytes.
