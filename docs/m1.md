...
- **Interception.** One WXT source tree (`src/extension`) builds both extensions.
  ...
  - Chrome redirects main-frame and sub-frame PDF responses to `capture.html?<PDF URL>`.
...
- **Capture.** The capture page asks the background to capture the PDF. In Firefox the background posts the bytes the filter kept; Chrome gives an extension no way to read a navigation's body, so there the background fetches the PDF again with the browser's cookies, and a single-use URL refuses that second request.
...
| single-use `/once/ticket.pdf` (403 after the first request) | Firefox: stored from its own response; Chrome: refusal shown | failure page with the 403 | `ticket.pdf` stored, one request |
