...
export async function capturePdf(
  ...
  // A PDF opened without a followed link (typed in, bookmarked, framed) is its own source.
  form.set("source_url", origin === undefined ? pdfUrl.href : origin.source_url);
  ...
  if (!posted.response.ok) {
    const detail = `${posted.response.status}: ${await posted.response.text()}`;
    return { kind: "failed", error: { stage: "post-bucket", detail } };
  }
