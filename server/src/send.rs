...
use crate::contract::{
    ...
    Resolution, SendResponse, SendResponsePerformedItem, SendSource, SendStep, Timestamp,
    ZoteroRecord, ZoteroStatus, ZoteroStatusSentPendingItem,
};
use crate::error::{AppError, AppResult};
use crate::index::IndexedItem;
use crate::organization::{filing_of, kept_offline, non_empty, remove_item, set_zotero_record};
...
async fn find_or_create(state: &Shared, indexed: &IndexedItem) -> AppResult<(ZoteroRecord, bool)> {
    ...
    let resolution = state
        .store
        .resolve(&stored.key, &state.config.resolvers_manifest)
        .await?;
