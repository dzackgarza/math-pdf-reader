...
| Path | What it is |
| --- | --- |
| `server/` | Rust (axum) bucket server: capture endpoint, PDF and reader URLs, library API, index export; `pdf-bucket` CLI (`serve`, `export-index`, `import-index`, `rebuild-cache`, `forget`) |
